...
Alfred Korzybski, George Orwell, and others have shown how living under the prolonged influence of phase-reversed terminology—where fundamental terms are inverted to signify their direct opposites and anti-rational contradictions are dogmatized as fundamental principles—creates a persistent, subconscious nervous tension that is unhealthy for the human organism. The human nervous system requires structural similarity between symbols and objective reality to evaluate facts calmly and rationally without cognitive dissonance. Consequently, absorbing, mastering, and employing correct theoretical physics—which replaces mystical verbal evasions with transparent, deterministic multivector field dynamics—eliminates this unnecessary nervous strain, restoring intellectual tranquility and improving neuro-physiological health (at least a little, we make no grand claims).
...
By replacing unphysical continuum point-particle singularities with non-singular Clifford multivector wave packets co-propagating across discrete lattice grid cells, the facts of solid-state physics are brought to proper status: one-time empirical laws that now are rigorously derived from first principles. The 'quantum' semiconductor fallacy is eliminated. Solid-state engineering finally rests on a firm foundation.
