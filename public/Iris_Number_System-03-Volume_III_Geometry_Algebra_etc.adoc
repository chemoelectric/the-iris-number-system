...
. **Quaternion Basis Mapping**: Identify basis bivectors as quaternionic units \( \mathbf { i } = - e _ 2 e _ 3 \), \( \mathbf { j } = - e _ 3 e _ 1 \), \( \mathbf { k } = - e _ 1 e _ 2 \). Direct computation yields:
\[ \mathbf { i } ^ 2 = ( - e _ 2 e _ 3 ) ( - e _ 2 e _ 3 ) = - e _ 2 e _ 3 e _ 2 e _ 3 = - \mathbf { 1 } _ { Cl } \]
\[ \mathbf { i } \mathbf { j } \mathbf { k } = ( - e _ 2 e _ 3 ) ( - e _ 3 e _ 1 ) ( - e _ 1 e _ 2 ) = - e _ 2 ( e _ 3 e _ 3 ) e _ 1 e _ 1 e _ 2 = - e _ 2 e _ 2 = - \mathbf { 1 } _ { Cl } \]
...
== Constructive 4D Homogeneous Geometric Algebra Cl(3,0,1) and Projective Rigid Motors

This section establishes 4D Homogeneous Projective Geometric Algebra \( Cl ( 3 , 0 , 1 ) \subset Cl ( 4 , 1 , 1 ) \) using spatial basis \( e _ 1 , e _ 2 , e _ 3 \) (\( e _ i ^ 2 = \mathbf { 1 } _ { Cl } \)) and homogeneous ideal origin basis \( e _ 0 \) with degenerate square \( e _ 0 ^ 2 = 0 \).

[#theorem-4d-homogeneous-motor]
[THEOREM]
.Theorem: Constructive 4D Homogeneous Projective Duality and Motor Invariance Theorem
====
In 4D Homogeneous Geometric Algebra \( Cl ( 3 , 0 , 1 ) \), points are represented as 1-vectors \( P = x e _ 1 + y e _ 2 + z e _ 3 + w e _ 0 \), planes as 1-vectors / 3-blades, and lines as 2-blades. Every rigid Euclidean motion (rotation + translation) on discrete grid \( \mathcal { G } _ N \) is represented by a dual-quaternionic motor \( M = T R \), where rotor \( R = \cos ( \theta / 2 ) \mathbf { 1 } _ { Cl } - \sin ( \theta / 2 ) B \) and translator \( T = \mathbf { 1 } _ { Cl } - \frac { 1 } { 2 } t e _ 0 \) (\( t = t _ x e _ 1 + t _ y e _ 2 + t _ z e _ 3 \)), operating via double-sided action \( P ' = M P \tilde { M } \).

*Proof:*

. **Translator Nilpotency**: Since \( e _ 0 ^ 2 = 0 \) and \( e _ 0 \) anti-commutes with spatial vectors \( e _ i \), the translation generator satisfies \( ( t e _ 0 ) ^ 2 = t e _ 0 t e _ 0 = - t ^ 2 e _ 0 ^ 2 = 0 \). Thus \( T = \exp ( - \frac { 1 } { 2 } t e _ 0 ) = \mathbf { 1 } _ { Cl } - \frac { 1 } { 2 } t e _ 0 \) is exact without higher order terms.

. **Translator Action on Homogeneous Points**: For normalized point \( P = x e _ 1 + y e _ 2 + z e _ 3 + e _ 0 \):
...
Since \( P e _ 0 + e _ 0 P = 2 e _ 0 e _ 0 + \dots = 0 + \text { spatial term } \), the spatial coordinates shift strictly to \( ( x + t _ x ) e _ 1 + ( y + t _ y ) e _ 2 + ( z + t _ z ) e _ 3 + e _ 0 \).

. **Motor Composition**: Motor \( M = T R \) satisfies \( M \tilde { M } = ( T R ) ( \tilde { R } \tilde { T } ) = T ( R \tilde { R } ) \tilde { T } = T \tilde { T } = \mathbf { 1 } _ { Cl } \), establishing exact rigid motor invariance on finite grid \( \mathcal { G } _ N \). \( \square \)
====
...
* xref:theorem-4d-homogeneous-motor[**Theorem: Constructive 4D Homogeneous Projective Duality and Motor Invariance Theorem**]  -- Chapter: *Constructive Clifford Geometry and Aperture Metrics* | Section: *Constructive 4D Homogeneous Geometric Algebra Cl(3,0,1) and Projective Rigid Motors*
